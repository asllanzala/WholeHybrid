...
+function ($) {
    ...
    $.modalConfirm = function (params) {
        ...
        var buttonsHTML = '';
        if (params.buttons && params.buttons.length > 0) {
            buttonsHTML += '<a id="giveup-btn" class="confirm-btn">' + params.buttons[0].text + '</a>';
            buttonsHTML += '<a id="sure-btn" class="confirm-btn">' + params.buttons[1].text + '</a>';
        }
        var extraClass = params.extraClass || '';
        var titleHTML = params.title ? '<div class="modal-title" id="modalConfirm-title">' + params.title + '</div>' : '';
        var textHTML = params.text ? '<div class="modal-text" id="modalConfirm-content">' + params.text + '</div>' : '';
        var afterTextHTML = params.afterText ? params.afterText : '';
        var noButtons = !params.buttons || params.buttons.length === 0 ? 'modal-no-buttons' : '';
        var verticalButtons = params.verticalButtons ? 'modal-buttons-vertical' : '';
        modalHTML = '<div class="modal ' + extraClass + ' ' + noButtons + '"><div class="modal-inner">' + (titleHTML + textHTML + afterTextHTML) + '</div>' +
            '<div class="modal-buttons ' + verticalButtons + '" id="modalConfirm-buttons">' + buttonsHTML + '</div></div>';
        ...
        // Add events on buttons
        modal.find('.confirm-btn').each(function (index, el) {
            $(el).on('click', function (e) {
                if (params.buttons[index].close !== false) $.closeModal(modal);
                if (params.buttons[index].onClick) params.buttons[index].onClick(modal, e);
                if (params.onClick) params.onClick(modal, index);
    ...
    $.confirm = function (text, title, callbackOk, callbackCancel, cancelText, sureText) {
        if (typeof title === 'function') {
        ...
        return $.modalConfirm({
            ...
            title: typeof title === 'undefined' ? defaults.modalTitle : title,
            buttons: [
                {text: cancelText != "" ? cancelText : defaults.modalButtonCancel, onClick: callbackCancel},
                {text: sureText != "" ? sureText : defaults.modalButtonOk, bold: true, onClick: callbackOk}
            ]
        });
    };
...
+function ($) {
    ...
    Router.prototype._init = function () {
        ...
        if (theHistory.state === null) {
            ...
            this._incMaxStateId();
//          console.log("保存curState="+curState);
        }
    };
     ...
     * @private
     */
    Router.prototype._back = function (state, fromState, isTheFragment) {
        //alert("dd");
        //console.log("state.url.full是"+state.url.full+"   fromState.url.full是"+fromState.url.full);
        if (this._isTheSameDocument(state.url.full, fromState.url.full)) {
            //console.log("两个 state 判断是属于同一个文档");
...
//      		console.log("两个 state 判断不是属于同一个文档");
            this._saveDocumentIntoCache($(document), fromState.url.full);
            var fragmentTag = state.pageId;
            console.log(fragmentTag);
//          changeOurHistory(state , fragmentTag , this);
            this._saveAsCurrentState(state);
            this._switchToDocument(state.url.full, false, false, DIRECTION.leftToRight);
        }
    };

//	function changeOurHistory (state , fragmentTag , _obj){
//		console.log(fragmentTag);
//		 switch (fragmentTag){
//          	case "scanProPage":
////          		alert("ddd");
////          		backHomePage（)
////          		 _obj._switchToDocument(state.url.base, false, false, DIRECTION.leftToRight);
//          		break;
//      		case  "sureOrderPage":
////      			$.confirm("Are you sure give up the order ? ", function(){
////						$.router.load("../other/home.html", true);
////				});
//      			break;
//          	default:
//          		 _obj._switchToDocument(state.url.full, false, false, DIRECTION.leftToRight);
//          		break;
//          }
//	}
    /**
    ...
    Router.prototype._onPopState = function (event) {
        ...
        if (state.id < lastState.id) {
            console.log("当前state=" + JSON.stringify(state) + "之前的state=" + JSON.stringify(lastState));
