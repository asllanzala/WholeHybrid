...
public class PriceDiffPaySelectActivity extends BaseActivity {
    ...
    private BaseTextView customerDebtTextView;
    ...
    public static PriceDiffPaySelectActivity priceDiffPaySelectActivity;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_price_diff_pay_select);
        priceDiffPaySelectActivity = this;
    ...
    public void onEventMainThread(PayRIghtNowEvent event) {
        if (event.getmMsg().equals(PriceDiffPresenter.SALE_RETURN_SUCCEED)) {
            finish();
        }
    }
    ...
    private void initView() {
        ...
        blankRelativeLayout.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                PriceDiffPaySelectActivity.this.finish();
            }
        });
        ...
        customerDebtTextView = (BaseTextView) findViewById(R.id.customer_debt_text_view);
        customerDebtTextView.setText(cartModel.getDebtDealsPrice());
        ...
        saveButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                saveButton.setEnabled(false);
                if (isEmpty()){
                    Toast.makeText(getApplicationContext(), getResources().getString(R.string.price_diff_money_empty), Toast.LENGTH_SHORT).show();
                    saveButton.setEnabled(true);
                }else if (compareDebt() == 1){
                    Toast.makeText(getApplicationContext(), getResources().getString(R.string.price_diff_money_over_debt), Toast.LENGTH_SHORT).show();
                    saveButton.setEnabled(true);
                }else if (compareMoney() != 0) {
                    Toast.makeText(getApplicationContext(), getResources().getString(R.string.price_diff_money_inequal), Toast.LENGTH_SHORT).show();
                    saveButton.setEnabled(true);
                } else {
                    getPayAccount();
                    priceDiffPresenter.traModel();
                    priceDiffPresenter.sareReturnOrder(saveButton);
                }
            }
        });
    ...
    private void initShow() {
        if (payAccountList != null) {
            if (switchButton.isChecked()) {
                ...
                debtTextView.setText(payAccountList.get(4).getAccountName() + "抵扣");
            } else {
                ...
                debtRadioButton.setText(payAccountList.get(4).getAccountName() + "抵扣");
                restTextView.setText("0");
            }
            saveButton.setEnabled(true);
        }
    ...
    public void jumpToMain() {
        Intent intent = new Intent(PriceDiffPaySelectActivity.this, MainActivity.class);
...
//        PriceDiffActivity.priceDiffActivity.finish();
    ...
    public void getPayAccount() {
        ...
        priceDiffPresenter.setPayAccountArrayList(payAccountOrderList);
    }
    ...
    private int compareDebt() {
        BigDecimal debtHave;
        BigDecimal debtTotalHave;
        if (switchButton.isChecked()) {
            if (debtEditText.getText().toString().equals("")) {
                debtHave = new BigDecimal("0");
            } else {
                debtHave = new BigDecimal(debtEditText.getText().toString());
                Log.e("wqdsas1",debtEditText.getText().toString());
            }
            if (customerDebtTextView.getText().toString().equals("")) {
                debtTotalHave = new BigDecimal("0");
            } else {
                debtTotalHave = new BigDecimal(customerDebtTextView.getText().toString());
                Log.e("wqdsas2",customerDebtTextView.getText().toString());
            }

            int comResult = debtHave.compareTo(debtTotalHave);
            return comResult;
        } else {
            if (debtRadioButton.isChecked()) {
                if (totalContentTextView.getText().toString().equals("")) {
                    ...
                    debtHave = new BigDecimal(totalContentTextView.getText().toString());
                    Log.e("wqdsas3",totalContentTextView.getText().toString());
                }
                if (customerDebtTextView.getText().toString().equals("")) {
                    debtTotalHave = new BigDecimal("0");
                } else {
                    debtTotalHave = new BigDecimal(customerDebtTextView.getText().toString());
                    Log.e("wqdsas4",customerDebtTextView.getText().toString());
                }
