    ...
    public static String getMessage(Object error) {
        if (error instanceof TimeoutError) {
            return StringUtil.getString(
                    R.string.http_request_generic_server_down);
        } else if (isServerProblem(error)) {
            return handleServerError(error);
        } else if (isNetworkProblem(error)) {
            return StringUtil.getString(R.string.http_request_no_internet);
        }
        return StringUtil.getString(R.string.http_request_generic_error);
    }
            ...
            switch (response.statusCode) {
                ...
                case 401:
                    try {
                        ...
                        HashMap<String, String> result = new Gson().fromJson(responseData, new TypeToken<Map<String, String>>() {}.getType());
                ...
                default:
                    LogHelper.getInstance().e(TAG, "Server Error: " + responseData);
                    return StringUtil.getString(
                            R.string.http_request_generic_server_down);
            }
        }
