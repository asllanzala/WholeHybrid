package com.honeywell.wholesale.framework.service.btservice;
...
public class BluetoothService {
    ...
    public static final int PRINTER_SUCCESS = 700;
    ...
    public static BluetoothService getInstance(Context context, Handler mHandler1) {
        if (sBluetoothService == null) {
            sBluetoothService = new BluetoothService(context, mHandler1);
        }
        mHandler = mHandler1;
...
//        printerCallBack.onBluetoothStateChanged(state);
    }
...
//        printerCallBack.onBluetoothConnectFailed();
...
//        EventBus.getDefault().post(new ConnectSuccessEvent(ConnectSuccessEvent.ConnectState.CONNECT_FAILE, "", "", null));
    ...
    private class ConnectThread extends Thread {
        ...
        public ConnectThread(BluetoothDevice device) {
            ...
            Log.e(TAG, "连接的设备名称：" + tmp.getRemoteDevice().getName());
        ...
        public void run() {
            ...
            try {
                ...
                Log.e(TAG, "连接异常" + var5.getLocalizedMessage());
    ...
    private class ConnectedThread extends Thread {
        ...
        public void run() {
            Log.d(TAG, "ConnectedThread线程运行" + "正在运行......");

            if (mHandler != null) {
                Message msg = mHandler.obtainMessage(PRINTER_SUCCESS);
            ...
            try {
                while (true) {
                    ...
                    if (mStringBuilder.toString().contains("RUN")) {
                        Log.e("=========每次处理后的原始数据============", "执行一次");
                        if (mHandler != null) {
                            Message msg = mHandler.obtainMessage(PRINTER_SUCCESS);
                            mHandler.sendMessage(msg);
                        }
...
//                        printerCallBack.onPrinterSuccess();
                    }
...
//                    if (isDEBUG)
//                        Log.e(TAG, "第一个条件" + mStringBuilder.toString().startsWith("Connected") + "第二个条件" + (mStringBuilder.toString().contains("{_CLC:HCD}") && mStringBuilder.toString().contains("Connected")));
//                    if (mStringBuilder.toString().startsWith("Connected") || (mStringBuilder.toString().contains("{_CLC:HCD}") && mStringBuilder.toString().contains("Connected"))) {
//                        mStringBuilder.delete(0, mStringBuilder.length() - 1);
